...
def binarize(image, sigma=0.6, crange=None, trange=[0.15, 0.6], csearch=False, thin=False):
    ...
    result, c, thi = common.algorithm2(image, clist=clist, csearch=csearch, thilist=trange, sigma=sigma, iter=20, thin=thin,         f=binarize_single)
        #print 'c=', c, 'thi=', thi
    return result
...
def binarize_single(image, thi=0.5, tlo=0.1, sigma=0.6, clist=[100], csearch=True, thin=False):
    ...
    image = numpy.pad(image, 2, mode='constant')
...
def howe_binarize(image):
    # image = cv2.imread(options.input_file)
    result = binarize(image, sigma=0.6,
                    crange=[60, 3000],
                    trange=[0.15, 0.6],
                    csearch=False,
                    thin=False)
...
def main():
    ...
    result = binarize(image, sigma=options.sigma,
                    ...
                    thin=options.thin)
    cv2.imwrite(options.output_file, result[1:-1,1:-1])
