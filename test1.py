...
import numpy as np
import pytesseract
from utils import Kuwahara,deskew, denoising, text_dilate, checkIfPointInsideArea
...
class OCR:
    ...
    def remove_border(self, img):
        ...
        th = cv2.threshold(img, 127, 255, cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU)[1]
        self.show(th)
        contours = cv2.findContours(th, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)[1]
        for c in contours:
            if 350 > cv2.contourArea(c) > 50:
                x, y, w, h = cv2.boundingRect(c)
                if coords[0] > x: coords[0] = x
                if coords[1] > y: coords[1] = y
                if coords[2] < x + w: coords[2] = x + w
                if coords[3] < y + h: coords[3] = y + h
        coords = np.array((coords[0], coords[1]-5, coords[2], coords[3] +5), dtype=int)
        coords[coords < 0] = 0
    ...
    def routine(self, img_dir,txt_file):
        ...
        regions = self.get_text_regions(enhaced)
        f = open(txt_file, "w+")
        ...
        for region in regions:
            ...
            crop = cv2.GaussianBlur(crop, (3,3), 3)
            crop = self.remove_border(crop)
            crop = deskew(crop)
            #crop = Kuwahara(crop.copy(), 5)
            #crop = cv2.threshold(crop, 127, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)[1]
            #crop = cv2.adaptiveThreshold(crop, 125, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 91, 12)
            #crop = self.denoising(crop,100)
            self.show(crop)
            text = self.predict(crop)
            if len(text) >4:

                f.write("Text of found on: x: {} y: {} x2: {} y2 {}\n ".format(region[0],region[1],region[2],region[3]) )
                f.write("\nSTART\n" +  text + "\nEND\n" )
        f.close()
